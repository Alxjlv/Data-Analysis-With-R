...
```
* Centre: Mean sleep hours for deprived is lower than mean sleep hours for non deprived as seen by the summary stats. 
* Spread: The spread is slightly wider (1.5) for non deprived as compared to deprived (1.0)
* Skew: Both Yes and No appear fairly symmetrical without too significant of a tail, so no significant skew. The boxplots for the two appear to be quite similar.
```{r}
layout20x(1,2)
...
The one-way plot we did alongside the EOV check shows that indeed there are clusters of values at certain levels, but our assumption of equality of variance is satisfied, as the plots appear to be spread similarly.
